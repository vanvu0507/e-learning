...
const getCurrentCourseProgress = async(req, res)=> {
    try {
        const {useId, courseId} = req.params;

        const studentPurchasedCourses = await StudentCourses.findOne({useId});

        const isCurrentCoursePurchasedByCurrentUserOrNot = studentPurchasedCourses?.courses?.findIndex(
            item=> item.courseId === courseId) > -1
        ...
        const currentUserCourseProgress = await CourseProgress.findOne({useId, courseId});
